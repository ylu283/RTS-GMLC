#!/bin/bash
#$ -N collect_stage2_C_n0b
#$ -q long
#$ -cwd

# Chained collector for stage2_C_n0b (prompt 27 n0b top-up; single-wave adaptation of the
...
#   2. on any gap: FAILED_stage2_C_n0b.md marker, commit + push the marker ONLY;
...
if [[ -n "$missing" ]]; then
    marker="$WAVE_DIR/FAILED_stage2_C_n0b.md"
    {
        echo "# FAILED — stage2_C_n0b incomplete at collection"
        echo
        ...
        echo "collector. Generated $(date -u +%FT%TZ) by collect_stage2_C_n0b.sh."
    } > "$marker"
    git -C "$REPO_DIR" add -- "$marker"
    git -C "$REPO_DIR" "${BOT[@]}" commit -m "stage2-bot: FAILED marker (stage2_C_n0b incomplete)"
    push_with_retries
    echo "GATE FAIL: missing indices: $missing — marker pushed, no partial CSVs."
    exit 1
fi
echo "GATE PASS: stage2_C_n0b complete ($(ls -d "$WAVE_DIR"/runs/run_index_* | wc -l) run dirs)"
...
git -C "$REPO_DIR" "${BOT[@]}" commit -m "stage2-bot: stage2_C_n0b results"
push_with_retries
echo "collect_stage2_C_n0b done: objectives pushed"
