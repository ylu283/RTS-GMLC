#!/bin/bash
#$ -N collect_pairgrid_tail_wind_317_C
#$ -q long
#$ -cwd

# Chained collector for pairgrid_tail_wind_317_C (prompt 28 T1; single-wave adaptation of the
...
#   2. on any gap: FAILED_pairgrid_tail_wind_317_C.md marker, commit + push the marker ONLY;
...
if [[ -n "$missing" ]]; then
    marker="$WAVE_DIR/FAILED_pairgrid_tail_wind_317_C.md"
    {
        echo "# FAILED — pairgrid_tail_wind_317_C incomplete at collection"
        echo
        ...
        echo "collector. Generated $(date -u +%FT%TZ) by collect_pairgrid_tail_wind_317_C.sh."
    } > "$marker"
    git -C "$REPO_DIR" add -- "$marker"
    git -C "$REPO_DIR" "${BOT[@]}" commit -m "pairgrid-bot: FAILED marker (pairgrid_tail_wind_317_C incomplete)"
    push_with_retries
    echo "GATE FAIL: missing indices: $missing — marker pushed, no partial CSVs."
    exit 1
fi
echo "GATE PASS: pairgrid_tail_wind_317_C complete ($(ls -d "$WAVE_DIR"/runs/run_index_* | wc -l) run dirs)"
...
git -C "$REPO_DIR" "${BOT[@]}" commit -m "pairgrid-bot: pairgrid_tail_wind_317_C results"
push_with_retries
echo "collect_pairgrid_tail_wind_317_C done: objectives pushed"
