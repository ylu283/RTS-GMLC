#!/bin/bash
# pairgrid_nuclear_wind_317_C — submit array + chained collector (prompt 28 T1).
...
J=$(qsub -terse -t 1-81 pairgrid_nuclear_wind_317_C_array.sh | cut -d. -f1)
[[ "$J" =~ ^[0-9]+$ ]] || { echo "BAD JID: $J"; exit 1; }
echo "pairgrid_nuclear_wind_317_C array submitted: J=$J (81 tasks, no -tc)"
qsub -hold_jid "$J" -cwd -M ylu28@nd.edu -m ea collect_pairgrid_nuclear_wind_317_C.sh
