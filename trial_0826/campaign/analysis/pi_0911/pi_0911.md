...
The diagnostic: draw each objective's **iso-value contour lines** in the
(PEM at 303, PEM at 317) plane [MW], and overlay dashed grey **iso-total
guides** (lines of equal total installed PEM, x + y = const). Reading
rule:

- contour lines **straight and parallel to the guides** → the objective
  depends only on the *total* MW — location is irrelevant, one knob
  suffices;
- contour lines **straight but tilted** → location matters, but
  trivially (one site always better per MW);
- contour lines **curved** → saturation/interaction — the *split*
  between sites genuinely matters and no single scalar knob captures it.

Each panel is annotated with two R² values on the 81 ground-truth
points: a planar fit (a·x + b·y + c) and a total-only fit (quadratic in
x + y). Contours are linear interpolation of the raw 9×9 (light dots =
the runs).
...
for i, (col, label, scale) in enumerate(OBJ):
    for j, t in enumerate(SCEN):
        ...
        # fits on the 81 points
        A = np.c_[x, y, np.ones_like(x)]
        coef, *_ = np.linalg.lstsq(A, z, rcond=None)
        r2_plane = r2(z, A @ coef)
        tot = x + y
        cq = np.polyfit(tot, z, 2)
        r2_total = r2(z, np.polyval(cq, tot))
        Zg = d.assign(v=z).pivot(index='wind_317_omega', columns='wind_303_omega', values='v')
        cs = ax.contour(Zg.columns * P303, Zg.index * P317, Zg.values,
                        levels=7, colors='tab:blue', linewidths=1.1)
        ...
        ax.annotate(f'R² plane {r2_plane:.2f} | total-only {r2_total:.2f}',
...
fig.suptitle('Iso-value contours vs iso-total guides (dashed): parallel = only total matters; curved = the split matters\n'
             '(contour labels in each objective\'s units; R² on the 81 ground-truth points)',
             fontsize=11, y=0.997)
fig.tight_layout(); plt.show()
```
...
**What it shows, objective by objective** *(numbers = the R² pairs in
the panels)*: **cost** contours run nearly parallel to the iso-total
guides at all three prices — cost is essentially a bill on total MW
withheld, location-blind. **Curtailment** contours are strongly curved
(planar R² drops well below the total-only fit's), bending around the
corner — each site saturates on its own curtailment pool, so the *split*
matters: spreading capacity recovers more energy than piling it on one
site. **Shed/reserve/starts** at B/C sit in between — mostly
total-driven with curvature at deep retrofit; at A they are
noise-plateau (contours meaningless there).

**Why this argues for multi-objective (and multi-site) optimization:**
the objectives disagree about *shape*, not just direction — cost prices
the total, curtailment rewards the split, reliability follows committed
capacity. No single knob (total MW) and no single objective ordering
captures all three at once; that is precisely the structure a
multi-objective optimizer over per-site sizes exists to navigate. (Per
review discipline: on this 2-site grid the tension is modest — the full
argument lives in the 6-tier space, where the sweeps show the same
per-site saturation at every tier and screening's +54.6% gap shows the
couplings compound.)
...
```
    ...
    /var/folders/p0/8_mj7nxn1td_x1r_61m5pqh40000gn/T/ipykernel_97083/1525470721.py:14: DeprecationWarning: Arrays of 2-dimensional vectors are deprecated. Use arrays of 3-dimensional vectors instead. (deprecated in NumPy 2.0)
      d = np.abs(np.cross(b - a, P - a)) / (np.linalg.norm(b - a) + 1e-12)
    /var/folders/p0/8_mj7nxn1td_x1r_61m5pqh40000gn/T/ipykernel_97083/1525470721.py:14: DeprecationWarning: Arrays of 2-dimensional vectors are deprecated. Use arrays of 3-dimensional vectors instead. (deprecated in NumPy 2.0)
      d = np.abs(np.cross(b - a, P - a)) / (np.linalg.norm(b - a) + 1e-12)
