...
### 8b. Ground-truth curves in whole-plant units — linear or not?

No heatmap here: these are the **raw ground-truth points** drawn as
curves. In every panel the x-axis is 303's capacity remaining as a
normal generator, (1 − ω₃₀₃) × 847 MW; each of the **9 curves is one
fixed level of 317** (color = 317's remaining normal generation,
light = intact, dark = fully behind the PEM bid); every marker is one
full-year run. Two things to read off directly: **curvature** of each
line (linear vs saturating response to 303's retrofit depth) and
**spacing between lines** (parallel & evenly spaced = the two plants
are additive; bunching = substitution/interaction).
...
lv317 = sorted(co['A'].wind_317_omega.unique())
cmap = cm.viridis
import matplotlib as mpl
cnorm = mpl.colors.Normalize(vmin=(1 - max(lv317)) * P317,
                             vmax=(1 - min(lv317)) * P317)
colors = {w: cmap(cnorm((1 - w) * P317)) for w in lv317}
fig, axes = plt.subplots(5, 3, figsize=(11.5, 16), sharex=True)
for i, (col, label, scale) in enumerate(OBJ):
    for j, t in enumerate(SCEN):
        ax = axes[i, j]
        for w in lv317:
            d = co[t][co[t].wind_317_omega == w].sort_values('wind_303_omega')
            ax.plot((1 - d.wind_303_omega) * P303, d[col] * scale, '-o',
                    color=colors[w], ms=3, lw=1.2)
        ax.axhline(0, color='k', lw=0.6)
        if i == 0:
            ax.set_title(f'{t}: B = {int(20 * SCEN[t])} \$/MWh (ρ = {SCEN[t]} \$/kg)', fontsize=10)
        if j == 0:
            ax.set_ylabel(label, fontsize=9)
sm = mpl.cm.ScalarMappable(cmap=cmap, norm=cnorm)
cb = fig.colorbar(sm, ax=axes, shrink=0.6, pad=0.015)
cb.set_label('317 capacity remaining as normal generation [MW]  (light = intact, dark = fully behind PEM bid)', fontsize=9)
fig.supxlabel('303_WIND_1 capacity remaining as normal generation [MW]  (nameplate 847)', fontsize=10, y=0.004)
fig.suptitle('Ground-truth curves in whole-plant units: every marker = one full-year run\n'
             '(9 curves per panel = the nine 317 levels)', fontsize=12, y=0.998)
plt.show()
```
...
![png](pi_0911_files/pi_0911_32_1.png)
    


*Reading note:* the curve shapes answer the linearity question metric by
metric. **Curtailment (g2)** is visibly convex — steep as 303's first
slices go behind the bid, flattening as the plant is stripped
(saturation) — and the 9 lines bunch toward dark colors: once 317 is
deep in, 303's marginal value shrinks (substitution, the +16–32%
interaction). **Cost (g1)** is close to linear in the retrofit depth at
all three prices, with mild flattening at deep retrofit. **Shed /
reserve / starts (g3–g5)** at B/C fall with retrofit depth on *both*
axes with visible curvature and line-bunching (one committed-capacity
effect); at A the lines sit on top of each other inside the noise floor
— a plateau, not a trend. Where lines are parallel and evenly spaced an
additive model would suffice; the systematic bunching is exactly why the
additive kernel was rejected.

### 8c. Does *where* the PEM sits matter? Contour-line view with iso-total guides

The diagnostic: each objective's **iso-value contour lines** in the
(PEM at 303, PEM at 317) plane [MW], with dashed grey **iso-total
guides** (equal total installed PEM). Reading rule, three branches:

- contours **parallel to the guides** → only the *total* matters (one
  knob suffices);
- contours **straight but tilted** → location matters, monotonically
  (one site is worth more per MW — the actionable 'which site first');
- contours **curved** → saturation/interaction — the *split* itself
  matters.

Each panel is annotated with the nested statistics that separate the
branches cleanly (expert-review upgrade — a plain plane-vs-total
comparison confounds curvature with location): **tot** = R² of a
quadratic in x+y (total-only, 3 params); **full** = R² of the full 2-D
quadratic (6 params); **Δloc** = full − tot, the clean location effect;
**tilt** = per-MW slope ratio 317/303 from the planar fit. Contours are
linear interpolation of the raw 9×9 (dots = the 81 runs); the
A-scenario reliability panels are greyed — their cell-level shapes sit
below the noise floor (contour spacing would be < the ±3 GWh floor).


```python
def r2(y, yhat):
    return 1 - np.sum((y - yhat)**2) / np.sum((y - y.mean())**2)
fig, axes = plt.subplots(5, 3, figsize=(11.5, 17), sharex=True, sharey=True)
GREY_A = {(2, 0), (3, 0), (4, 0)}  # A-scenario shed/reserve/starts: sub-floor shapes
for i, (col, label, scale) in enumerate(OBJ):
    for j, t in enumerate(SCEN):
        ax = axes[i, j]
        d = co[t]
        x = d.wind_303_omega.values * P303
        y = d.wind_317_omega.values * P317
        z = d[col].values * scale
        # nested fits (expert-review statistics)
        A1 = np.c_[x, y, np.ones_like(x)]
        a, b, _ = np.linalg.lstsq(A1, z, rcond=None)[0]
        tilt = b / a if abs(a) > 1e-12 else np.nan
        tot = x + y
        r2_tot = r2(z, np.polyval(np.polyfit(tot, z, 2), tot))
        Aq = np.c_[np.ones_like(x), x, y, x**2, y**2, x*y]
        r2_full = r2(z, Aq @ np.linalg.lstsq(Aq, z, rcond=None)[0])
        dloc = r2_full - r2_tot
        Zg = d.assign(v=z).pivot(index='wind_317_omega', columns='wind_303_omega', values='v')
        grey = (i, j) in GREY_A
        cs = ax.contour(Zg.columns * P303, Zg.index * P317, Zg.values, levels=7,
                        colors='lightgrey' if grey else 'tab:blue',
                        linewidths=0.8 if grey else 1.1)
        if not grey:
            ax.clabel(cs, fontsize=6, fmt='%.0f')
        else:
            ax.annotate('cell-level shapes\n< noise floor', xy=(0.5, 0.55),
                        xycoords='axes fraction', ha='center', fontsize=8, color='crimson')
        for tt in np.linspace(300, 1500, 5):
            ax.plot([0, tt], [tt, 0], '--', color='grey', lw=0.7, alpha=0.6)
        ax.plot(x, y, '.', color='k', ms=1.5, alpha=0.3)
        ax.set_xlim(0, 870); ax.set_ylim(0, 820)
        ax.annotate(f'tot {r2_tot:.2f} | full {r2_full:.2f} | Δloc {dloc:.2f} | tilt {tilt:.1f}×',
                    xy=(0.03, 0.03), xycoords='axes fraction', fontsize=7,
                    bbox=dict(boxstyle='round,pad=0.2', fc='white', ec='none', alpha=0.85))
        if i == 0:
            ax.set_title(f'{t}: B = {int(20 * SCEN[t])} \$/MWh', fontsize=10)
        if j == 0:
            ax.set_ylabel(f'{label}\nPEM at 317 [MW]', fontsize=8)
for j in range(3):
    axes[4, j].set_xlabel('PEM at 303 [MW]')
fig.suptitle('Iso-value contours vs iso-total guides (dashed): parallel = total-only; tilted = one site worth more per MW; curved = the split matters\n'
             '(tot/full = nested R²; Δloc = location effect; tilt = per-MW slope ratio 317/303)',
...
![png](pi_0911_files/pi_0911_35_0.png)
    


**What the pair grid does — and does not — license for the MOBO case**
*(numbers from the panel annotations and the expert-review recomputation)*

- **Cost**: location-blind at A only (tilt 1.0, per-MW slopes within
  1.3%). At B/C it is the *straight-but-tilted* branch: **a MW behind
  the bid at 317 costs ≈ 1.5× a MW at 303** (tilt 1.5; plane beats
  total-only, 0.97 vs 0.93).
- **Curtailment** is the one genuinely symmetric-and-curved surface
  (tilt ≈ 1.0, Δloc from curvature): at a fixed ~850 MW budget,
  spreading capacity across the two sites recovers **~250 GWh more**
  than piling it on one (≫ the 5 GWh floor) — each site saturates on
  its own curtailment pool.
- **Shed / starts at B/C**: mostly *tilt*, not curvature — per MW, 317
  buys **≈ 1.3–1.9×** the reliability of 303. So cost and the
  reliability cluster disagree not only on *how much* total but on
  *which site first* (at a fixed budget: reliability prefers
  317-leaning/balanced splits, cost prefers 303-heavy — worth $11–13M
  across the split range).
- **A-row reliability panels are greyed**: cell-level contour shapes
  sit below the ±3 GWh floor. (The *aggregate* 317-axis trend is real —
  shed swings ~6 GWh along 317, 2× the floor, vs ~1.6 GWh along 303 —
  but no within-panel geometry should be read.)

**The two-premise version of the optimization argument** (expert-review
corrected): the objective *conflict* itself is a matter of direction and
was established elsewhere — the cost sign flip and the opposite-corner
optima (cost's argmin and the reliability cluster's argmin sit ~1,100 MW
apart at B/C). **What this figure adds is that the conflict cannot be
arbitrated on a single total-MW knob**: no objective is exactly a
function of the total (the one-knob reduction leaks 5–10% of variance,
Δloc 0.04–0.10 at B/C), and at a fixed budget the split moves
curtailment and cost in opposite directions. On this 2-site grid that
residual is modest — per review discipline, the *necessity* argument for
MOBO in the full 6-tier space rests on the sweeps' per-tier saturation
and screening's +54.6% interaction gap; this figure corroborates the
mechanism where it can be measured exactly.
...
```
    ...
    /var/folders/p0/8_mj7nxn1td_x1r_61m5pqh40000gn/T/ipykernel_99192/1525470721.py:14: DeprecationWarning: Arrays of 2-dimensional vectors are deprecated. Use arrays of 3-dimensional vectors instead. (deprecated in NumPy 2.0)
      d = np.abs(np.cross(b - a, P - a)) / (np.linalg.norm(b - a) + 1e-12)
    /var/folders/p0/8_mj7nxn1td_x1r_61m5pqh40000gn/T/ipykernel_99192/1525470721.py:14: DeprecationWarning: Arrays of 2-dimensional vectors are deprecated. Use arrays of 3-dimensional vectors instead. (deprecated in NumPy 2.0)
      d = np.abs(np.cross(b - a, P - a)) / (np.linalg.norm(b - a) + 1e-12)
...
![png](pi_0911_files/pi_0911_38_1.png)
...
![png](pi_0911_files/pi_0911_39_0.png)
