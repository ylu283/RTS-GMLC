...
### 8b. The same atlas in whole-plant units — remaining normal generation [MW]

Identical data, re-axed in **whole-plant terms**: each axis is the wind
capacity still operating as a normal generator, (1 − ω) × nameplate
(nameplates read live from `tiers.py` and printed below). Reading frame:
the **top-right corner ≈ the plants as they are today** (nearly all of
303's 847 MW and 317's 799 MW selling as ordinary wind); moving toward
the **origin dedicates the plant to the PEM bid** until, at (0, 0), both
plants sit entirely behind B. The patterns are §8 mirrored through the
corner — improvements now deepen toward the origin.
...
for i, (col, label, scale) in enumerate(OBJ):
    ...
    for j, t in enumerate(SCEN):
        Z = grids[t]
        xg = (1.0 - Z.columns.values) * P303   # remaining normal generation, MW
        yg = (1.0 - Z.index.values) * P317
        pm = axes[i, j].pcolormesh(xg, yg, Z.values, norm=norm,
                                   cmap='RdBu_r', shading='nearest')
        if i == 0:
            axes[0, j].set_title(f'{t}: B = {int(20 * SCEN[t])} \$/MWh\n(ρ = {SCEN[t]} \$/kg)', fontsize=10)
    cb = fig.colorbar(pm, ax=axes[i, :], shrink=0.85, pad=0.015)
    cb.set_label(label, fontsize=9)
fig.supxlabel('303_WIND_1 capacity remaining as normal generation [MW]  (nameplate 847)', fontsize=10, y=0.005)
for i in range(5):
    axes[i, 0].set_ylabel('317 normal gen [MW]')
fig.suptitle('Wind-pair objective atlas in whole-plant units: capacity remaining as normal generation\n'
             '(top-right ≈ plants as-is; origin = both plants fully behind the PEM bid; blue = improvement)',
             fontsize=12, y=0.995)
plt.show()
```
...
*Reading note:* the structure is §8 mirrored — improvements deepen as
normal generation shrinks toward the origin. This frame answers the
operator-flavored question directly: **how much ordinary wind can the
system keep while collecting the benefits?** The B/C reliability corner
requires stripping both plants nearly bare (< 100 MW each left as normal
generation), while the §9 knee design keeps 303 essentially intact
(~805 of 847 MW) and leaves ~288 MW of 317 — most of the shed reduction
is bought while 303 stays an ordinary wind farm.
...
```
    ...
    /var/folders/p0/8_mj7nxn1td_x1r_61m5pqh40000gn/T/ipykernel_79329/1525470721.py:14: DeprecationWarning: Arrays of 2-dimensional vectors are deprecated. Use arrays of 3-dimensional vectors instead. (deprecated in NumPy 2.0)
      d = np.abs(np.cross(b - a, P - a)) / (np.linalg.norm(b - a) + 1e-12)
    /var/folders/p0/8_mj7nxn1td_x1r_61m5pqh40000gn/T/ipykernel_79329/1525470721.py:14: DeprecationWarning: Arrays of 2-dimensional vectors are deprecated. Use arrays of 3-dimensional vectors instead. (deprecated in NumPy 2.0)
      d = np.abs(np.cross(b - a, P - a)) / (np.linalg.norm(b - a) + 1e-12)
