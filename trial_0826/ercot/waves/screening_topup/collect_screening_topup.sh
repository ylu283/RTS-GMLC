#!/bin/bash
#$ -N collect_screening_topup
#$ -q long
#$ -cwd

# Chained collector for the ERCOT screening TOP-UP wave (prompt 26 T4;
# prompt-27 collector pattern, deltas: PCM_ERCOT env, ercot/tx123 push).
# Submitted from INSIDE waves/screening_topup by ercot_block_4b.sh:
#   qsub -hold_jid "$JID" -cwd -M ylu28@nd.edu -m ea collect_screening_topup.sh
...
# rider-(i) ruc_quality.csv (array task logs screening_topup_array.o*.N),
...
if [[ -n "$missing" ]]; then
    marker="$WAVE_DIR/FAILED_screening_topup.md"
    {
        echo "# FAILED — screening_topup incomplete at collection"
        echo
        echo "Missing run indices (no overall_simulation_output.csv):"
        echo
        for i in $missing; do echo "- $i"; done
        echo
        echo "Recovery: manual 'qsub -t a-b screening_topup_array.sh' per"
        echo "contiguous gap, then re-run this collector. Generated"
        echo "$(date -u +%FT%TZ) by collect_screening_topup.sh."
    } > "$marker"
    echo "GATE FAIL: missing indices: $missing"
    git -C "$REPO_DIR" add -- "$marker"
    git -C "$REPO_DIR" "${BOT[@]}" commit -m "ercot-bot: FAILED marker (screening_topup incomplete)"
    push_with_one_retry
    exit 1
fi
echo "GATE PASS: all 7 runs complete"
...
while IFS=, read -r i _; do
    ...
    OUT_DIR="$ERCOT_DIR/extracts/screening_topup/run_index_$i"
    JOBLOG=$(ls -t "$WAVE_DIR"/screening_topup_array.o*."$i" 2>/dev/null | head -1 || true)
...
git -C "$REPO_DIR" "${BOT[@]}" commit -m "ercot-bot: screening_topup results (curtailment-arm extracts)"
push_with_one_retry
echo "collect_screening_topup done: extracts pushed to ercot/tx123"
